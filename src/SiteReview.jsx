...
export function SiteReviewDialog({site,onClose,onEdit,onResolve}){
 ...
 return <div className="studio-overlay review-overlay" onClick={event=>{if(event.target===event.currentTarget)onClose()}}><section className="review-dialog" ref={dialog} role="dialog" aria-modal="true" aria-labelledby="review-title" onKeyDown={event=>{
  ...
  if(event.key==='Tab'){const buttons=[...dialog.current.querySelectorAll('button:not(:disabled),summary')].filter(node=>node.getClientRects().length),first=buttons[0],last=buttons.at(-1);if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}}
 ...
 <div className="review-scroll"><div className="review-pages">{languages.map(page=><div key={page.lang}><strong>{page.lang==='en'?'EN':'KOR'}</strong><span>{page.visible}개 섹션 공개{page.excluded.length>0&&<details className="review-excluded"><summary>공개되지 않는 섹션 {page.excluded.length}개</summary><ul>{page.excluded.map(item=><li key={item.sectionId}><strong>{item.name}</strong><small>{item.reason}</small><button type="button" onClick={()=>onEdit({...item,lang:page.lang})}>{item.hidden?'설정 위치로 이동':'내용 입력하기'}</button></li>)}</ul></details>}</span></div>)}</div>
