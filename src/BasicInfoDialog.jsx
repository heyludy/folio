...
export function BasicInfoDialog({site,creating=false,initialLanguage='en',onSave,onClose}){
 ...
 return <div className="studio-overlay" onClick={e=>{if(e.target===e.currentTarget)onClose()}}><form ref={dialog} className={`basic-dialog ${step==='templates'?'template-dialog':step==='languages'?'language-dialog':''}`} role="dialog" aria-modal="true" aria-labelledby="basic-title" aria-describedby="basic-description" onSubmit={e=>{e.preventDefault();if(step==='templates')setStep('languages');else if(step==='languages')next();else onSave(draft,languages,resolveTemplate(templateId).id)}} onKeyDown={e=>{
  ...
  {step==='templates'?<>
   ...
   {creating?<div className="quick-project-languages">{[['english','영어만'],['bilingual','영어 + 한글']].map(([value,label])=><label key={value}><input type="radio" name="quick-language" value={value} checked={mode===value} onChange={()=>{setMode(value);setLanguage('en')}}/>{label}</label>)}</div>:<div className="basic-site-language"><span>홈페이지 언어 <strong>{mode==='bilingual'?'영어 + 한글':'영어만'}</strong></span><button type="button" onClick={()=>setStep('languages')}>변경</button></div>}
   ...
   <footer><button type="button" className="studio-button" onClick={onClose}>취소</button><button type="submit" className="studio-button primary">{creating?'사이트 만들기':'전체에 적용'}</button></footer>
  </>}
 </form></div>;
}
