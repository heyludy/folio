 ...
 return <main className="project-home"><div className="project-heading"><div><h1>프로젝트<span>{sites.length}</span></h1><p>교수님의 연구와 이야기를 한 페이지에.</p></div><div className="project-heading-actions"><a className="studio-button" href="./examples/" target="_blank" rel="noopener noreferrer">템플릿 예시 4종<ArrowUpRight size={14}/></a><button className="studio-button primary" onClick={onCreate}><Plus size={16}/>새 프로젝트</button></div></div>
  <div className="project-grid">{sites.map(site=>{
   ...
   return <article className="project-card" key={site.id}><button type="button" className="project-open" onClick={()=>onOpen(site.id)} aria-label={`${title} 프로젝트 열기`}>
    ...
    <div className="project-card-info"><div>{site.example&&<span className="project-example">Folio 예시 프로젝트</span>}<h2>{title}</h2><p>{ko.college||en.college||site.setup?.affiliation||'이름과 소개를 채워보세요'}</p><span className="project-template-label">{resolveTemplate(site.template).name}</span></div></div>
   ...
   {changed&&<p className="project-unpublished-note">{actionsDisabled?'변경사항 저장을 확인한 뒤 게시해 주세요.':'저장된 수정사항은 아직 공개 사이트에 반영되지 않았어요.'}</p>}
   ...
   <div className="project-card-actions">
    <button type="button" className="project-edit" onClick={()=>onOpen(site.id)} aria-label={`${title} ${site.setup?.stage==='content'?'자료 준비':site.setup?.stage==='media'?'사진·CV':site.setup?.stage==='template'?'디자인 선택':'편집하기'}`}><Pencil size={13}/>{site.setup?.stage==='content'?'자료 준비':site.setup?.stage==='media'?'사진·CV':site.setup?.stage==='template'?'디자인 선택':'편집하기'}</button>
    {url&&<a href={url} target="_blank" rel="noopener noreferrer" aria-label={`${title} 바로가기`}><ArrowUpRight size={14}/>바로가기</a>}
    <button type="button" disabled={actionsDisabled||publication?.checking||info?.kind==='pending'} onClick={()=>checkable?onCheckLink(site.id):onPublish(site.id)} aria-label={`${title} ${checkable?'게시 주소 확인':changed?'변경사항 게시':url?'게시 설정':'게시하기'}`}><Upload size={13}/>{checkable?'주소 확인':changed?'변경사항 게시':url?'게시 설정':'게시하기'}</button>
  ...
  })}<button className="project-new" onClick={onCreate}><span><Plus size={23}/></span><strong>새 프로젝트</strong><small>기본 정보부터 자료·디자인까지</small></button></div>
