...
import {templatePages,usesTemplatePages} from './templateLayout';
...
function Portrait({section,lang,photo,editing,onPhoto,photoLayout}){
...
function Content({section:s,lang,editing,onEdit,onEntry,onAsset,photo,onPhoto,photoLayout,profileLinks,template}){
 ...
 if(s.kind==='profile'){
  ...
  if(template==='color')return <div className={`site-color-profile ${photo||editing?'':'no-photo'}`}>
   <div className="site-color-hero">
...
export function SitePage({site,lang='en',editing=false,thumbnail=false,selected,selectedElement,onSelectElement,onElementResize,onEdit,onEntry,onSelect,onLanguage,onAddLanguage,onRemoveLanguage,onAdd,onMove,onHide,onDelete,onMenu,onPhoto,onAsset,onBasics,dragProps=()=>({})}){
 ...
 const template=resolveTemplate(site.template).id,paginated=usesTemplatePages(template);
 const pages=templatePages(sections),pageFor=Object.fromEntries(pages.flatMap(page=>page.sections.map(section=>[section.id,page.id])));
 const navSections=paginated?pages.map(page=>page.section):sections.filter(s=>s.nav);
 ...
 return <ElementEditor.Provider value={{editing,selected:selectedElement?.section===selected?selectedElement:null,onSelect:onSelectElement,onResize:onElementResize}}><div className="faculty-site" lang={lang} style={themeStyle(site,lang)} data-template={template} data-editing={editing} data-paginated={paginated&&!editing||undefined}>
  ...
  <nav className="site-nav" aria-label="홈페이지 메뉴" onKeyDown={e=>{if(e.key==='Escape'&&menuOpen){setMenuOpen(false);menuButton.current?.focus()}}}>
   {template==='color'&&profile&&<a className="site-nav-identity" href={`#${lang}-section-${profile.id}`}>{profile.text[lang].title}</a>}
   {template==='sidebar'&&<div className="site-sidebar-identity" aria-hidden="true"><strong>{footer.name}</strong><span>{footer.college}</span></div>}
   ...
   <div className="site-navlinks" id={`${lang}-site-navlinks`} data-open={menuOpen}>{navSections.map(s=><a key={s.id} href={`#${lang}-section-${s.id}`} onClick={()=>{setMenuOpen(false);if(menuButton.current?.getClientRects().length)menuButton.current.focus({preventScroll:true})}}>{navigationLabel(s,lang)}</a>)}</div>
  ...
  <div className="site-sections">{(thumbnail?sections.slice(0,3):sections).map((s,index)=><React.Fragment key={s.id}><section id={`${lang}-section-${s.id}`} className="site-section" data-section={s.id} data-kind={s.kind} data-template-page={paginated?pageFor[s.id]:undefined} hidden={thumbnail&&paginated&&pageFor[s.id]!==pages[0]?.id||undefined} data-active={editing&&s.id===selected&&selectedElement?.section!==s.id} data-reveal={!editing?'':undefined} {...(editing?dragProps(s):{})} onFocus={e=>{if(editing&&!e.target.closest('[data-element-edit]'))onSelect(s.id)}} onClick={e=>{if(editing&&!e.target.closest('[data-element-edit]'))onSelect(s.id)}}>
    {editing&&<div className="site-tools"><label><input type="checkbox" checked={s.nav} onChange={e=>onMenu(s.id,e.target.checked)}/>{paginated?'별도 페이지로 표시':'상단 메뉴에 표시'}</label><button type="button" aria-label={`${s.name} 위로 이동`} disabled={index===0} onClick={()=>onMove(s.id,-1)}>↑</button><button type="button" aria-label={`${s.name} 아래로 이동`} disabled={index===sections.length-1} onClick={()=>onMove(s.id,1)}>↓</button><button type="button" onClick={()=>onHide(s.id)}>페이지에서 숨기기</button><button type="button" aria-label={`${s.name} 삭제`} onClick={()=>onDelete?.(s.id)}>삭제</button></div>}
    {editing&&<button className="site-grip" type="button" aria-label={`${s.name} 끌어서 이동`} data-grip={s.id}>⠿</button>}
    <Content section={s} lang={lang} editing={editing} onEdit={onEdit} onEntry={onEntry} onAsset={onAsset} photo={site.photo} onPhoto={onPhoto} photoLayout={site.photoLayout} profileLinks={profileLinks} template={template}/>
   </section>{index<sections.length-1&&(editing?<div className="site-between"><button type="button" onClick={()=>onAdd(s.id)} aria-label={`${s.name} 다음에 섹션 추가`}>+</button></div>:(!paginated||pageFor[s.id]===pageFor[sections[index+1]?.id])&&<div className="site-rule" data-template-page={paginated?pageFor[s.id]:undefined} hidden={thumbnail&&paginated&&pageFor[s.id]!==pages[0]?.id||undefined}/>)}</React.Fragment>)}{editing&&available&&<div className="site-add-section" data-empty={!sections.length}><button type="button" onClick={()=>onAdd(sections.at(-1)?.id||'')} aria-label={!sections.length?'첫 섹션 추가':'맨 아래에 섹션 추가'}><span aria-hidden="true">+</span>섹션 추가</button></div>}</div>
