...
export function SitePage({site,lang='en',editing=false,thumbnail=false,selected,selectedElement,onSelectElement,onElementResize,onEdit,onEntry,onSelect,onLanguage,onAddLanguage,onRemoveLanguage,onAdd,onMove,onHide,onDelete,onMenu,onPhoto,onAsset,onBasics,dragProps=()=>({})}){
 ...
 const template=resolveTemplate(site.template).id;
 ...
 return <ElementEditor.Provider value={{editing,selected:selectedElement?.section===selected?selectedElement:null,onSelect:onSelectElement,onResize:onElementResize}}><div className="faculty-site" lang={lang} style={themeStyle(site,lang)} data-template={template} data-editing={editing}>
  {site.example&&<aside className="site-example-note"><span>{site.example.fictional?(lang==='en'?'Folio template demo · Fictional profile':'Folio 템플릿 예시 · 가상의 인물과 자료입니다'):(lang==='en'?'Folio example · Unofficial demo':'Folio 예시 프로젝트 · 공식 홈페이지가 아닙니다')}</span>{safeLink(site.example.official)&&<a href={safeLink(site.example.official)} target="_blank" rel="noopener noreferrer">{lang==='en'?'Official website ↗':'공식 홈페이지 ↗'}</a>}</aside>}
  <nav className="site-nav" aria-label="홈페이지 메뉴" onKeyDown={e=>{if(e.key==='Escape'&&menuOpen){setMenuOpen(false);menuButton.current?.focus()}}}>
   ...
   <button type="button" ref={menuButton} className="site-menu-toggle" aria-label={lang==='en'?'Toggle navigation':'메뉴 열기·닫기'} aria-expanded={menuOpen} aria-controls={`${lang}-site-navlinks`} onClick={()=>setMenuOpen(v=>!v)}><span aria-hidden="true">☰</span><span className="site-menu-current">{lang==='en'?'Menu':'메뉴'}</span></button>
  ...
  {!thumbnail&&available&&(showFooter||editing)&&<footer className="site-footer" aria-label={lang==='en'?'Website footer':'홈페이지 푸터'}>
   {filled(footer.name)&&<p className="site-copyright">{site.example?(site.example.fictional?(lang==='en'?'Folio demo · All content is fictional':'Folio 예시 · 모든 내용은 가상 자료입니다'):(lang==='en'?'Folio example · Public-source biography':'Folio 예시 · 공개 자료로 구성')):`© ${new Date().getFullYear()} ${footer.name}. All rights reserved.`}</p>}
