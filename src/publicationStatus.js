...
export function publicationStatus(state, comparison, site){
 ...
 if(state.liveSourceHash&&comparison.sourceHash!==state.liveSourceHash)return {label:'공개 사이트에 미반영',kind:'content',needsPublish:true,detail:'수정한 내용이 아직 공개 사이트에 반영되지 않았어요. 변경사항을 게시해 주세요.'};
 if(!comparison.hash)return {label:'게시 상태 확인 중',kind:'checking',needsPublish:false};
 if(comparison.hash!==state.liveHash)return {label:'페이지 표시 업데이트 필요',kind:'design',needsPublish:true,detail:state.liveSourceHash?'내용은 같지만 디자인이나 공유 이미지가 달라졌어요. 변경사항을 게시해 반영하세요.':'현재 페이지와 게시된 페이지가 달라요. 미리보기 후 변경사항을 게시하세요.'};
