...
export function exportSite(site,{shareImage=''}={}){
 ...
 return `<!doctype html>\n<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="description" content="${esc(info.description)}"><title>${esc(title)}</title>${shareHead(info,shareImage)}<style>${siteCss}</style><noscript><style>.site-navlinks{display:flex!important}.site-menu-toggle,[data-pdf=download]{display:none!important}</style></noscript></head><body class="public-body" style="--public-paper:${(themes[site.theme]||themes.forest).paper}">${content}<script>${script}</script></body></html>`;
}
