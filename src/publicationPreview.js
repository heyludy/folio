...
export function preparePublication(site){
 if(!previews.has(site)){
  const promise=createShareCard(site).then(async image=>({image,bundle:{...await publishBundle(exportSite(site,{shareImage:image})),sourceHash:await siteFingerprint(site)}})).catch(error=>{previews.delete(site);throw error});
