 ...
 return <div className="template-options" role="radiogroup" aria-label="프로젝트 템플릿">
  {templates.map(template=>{
   ...
   return <label className="template-option" key={template.id} data-selected={ready&&value===template.id} data-planned={!ready}>
    ...
    {ready?<div className="template-miniature" aria-hidden="true"><div className="template-mini-nav"><i/><i/><i/></div><div className="template-mini-profile"><div><strong>Professor Name</strong><span/><span/></div><div className="template-mini-photo"/></div><div className="template-mini-columns"><div><i/><span/><span/></div><div><i/><span/><span/></div></div></div>:<div className="template-coming" aria-hidden="true"><Icon size={30} strokeWidth={1.2}/></div>}
    <div className="template-option-title"><strong>{template.name}</strong><span className="template-status">{ready?'현재 템플릿':'추가 예정'}</span></div>
