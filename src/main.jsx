...
import {Plus,Eye,ArrowLeft,Download,Check,GripVertical,X,Palette,ContactRound,FileText,Globe,History,LayoutTemplate,ImagePlus} from 'lucide-react';
...
import {SetupMediaDialog} from './SetupMediaDialog';
import {PublicationNotice} from './PublicationActions';
import {setSetupCv,setSetupPhoto} from './setupMedia';
import {createPreparedProject,finishContentSetup,startManualSetup,finishMediaSetup,finishTemplateSetup} from './projectSetup';
...
function App({account,onLogout}){
 ...
 const [creating,setCreating]=useState(false),[templating,setTemplating]=useState(false),[settingMedia,setSettingMedia]=useState(false);
 ...
 const loadLatest=async()=>{
  ...
  try{
   ...
   setCreating(false);setTemplating(false);setSettingMedia(false);setPreparing(false);setPublishing(false);setImportUndo(null);setSelected(null);setSelectedElement(null);setPendingNavigation(null);setRemovedSection(null);setRemovedLanguage(null);setRemovedProject(null);setBasicsModal(null);setInsertAfter(null);
 ...
 useEffect(()=>{if(ready&&!availableSites.some(s=>s.id===siteId)){setHome(true);setCreating(false);setTemplating(false);setSettingMedia(false);setPreparing(false);setPublishing(false);setBasicsModal(null);setPreview(false);setSiteId(availableSites[0]?.id??null)}},[ready,sites,siteId]);
 ...
 const changeSite=id=>{const target=latestSites.current.find(s=>s.id===id);setSettingMedia(target?.setup?.stage==='media');setTemplating(target?.setup?.stage==='template');setPreparing(target?.setup?.stage==='content');setPublishing(false);setPendingNavigation(null);endDrag(false);setSiteId(id);setLang('en');setHome(false);setPreview(false);setSelected(null);setInsertAfter(null);setPaletteOpen(false);canvas.current?.scrollTo({top:0,behavior:'instant'})};
 const goHome=()=>{setSettingMedia(false);setTemplating(false);setCreating(false);setPreparing(false);setPublishing(false);setPendingNavigation(null);endDrag(false);setHome(true);setPreview(false);setPaletteOpen(false);setSelected(null);setInsertAfter(null);history.replaceState(null,'',location.pathname+location.search)};
 ...
 const createSite=()=>setCreating(true);
 const createPrepared=info=>{const next=createPreparedProject(info);setSites(all=>[...all,next]);setCreating(false);changeSite(next.id);setPreparing(true)};
 const openTemplates=()=>{endDrag();setPaletteOpen(false);setSelected(null);setSelectedElement(null);setTemplating(true)};
 const saveTemplate=id=>{update(s=>finishTemplateSetup(applyTemplateDesign(s,id)));setTemplating(false);setSelected(null);setSelectedElement(null);setPreview(false);setSaveMessage('디자인을 적용했어요. 내용은 그대로 유지돼요.');canvas.current?.scrollTo({top:0,behavior:'instant'})};
 const openMedia=()=>{endDrag();setPaletteOpen(false);setSelected(null);setSelectedElement(null);setSettingMedia(true)};
 const saveSetupMedia=transform=>{const target=latestSites.current.find(s=>s.id===site.id);if(!target||target.deletedAt)throw new Error('프로젝트가 삭제되었어요.');const next=transform(target);setSites(all=>all.map(s=>s.id===target.id?next:s))};
 const continueFromMedia=()=>{update(finishMediaSetup);setSettingMedia(false);setTemplating(true)};
 const manualSetup=()=>{update(startManualSetup);setPreparing(false);setSettingMedia(true)};
 const saveBasics=(draft,languages,icon)=>{
  setSites(all=>all.map(s=>s.id===basicsModal.site.id?{...applyBasicInfo(s,draft),languages,icon}:s));if(!languages.includes(lang))setLang('en')
  setBasicsModal(null);
 };
 ...
 const applyPrepared=async(next,result)=>{
  ...
  let merged;try{merged=finishContentSetup(mergeDrafts(site,next,current))}catch{return false;}
  setImportUndo({siteId:site.id,before:current,after:merged});setImportProblem('');setRemovedSection(null);setRemovedLanguage(null);
  setSettingMedia(merged.setup?.stage==='media');setTemplating(merged.setup?.stage==='template');setImportResult({siteId:site.id,...result,at:Date.now()});setRemovedEntry(null);update(()=>merged);setPreparing(false);setPublishing(false);setSelected(null);setSelectedElement(null);setPreview(false);
 ...
 return <div className="studio"><div className="studio-background" inert={!!(creating||templating||settingMedia||basicsModal||preparing||publishing||importBusy||historySite||reviewing||imageUpload)||undefined} aria-hidden={!!(creating||templating||settingMedia||basicsModal||preparing||publishing||importBusy||historySite||reviewing||imageUpload)||undefined}>
  ...
  {home?<ProjectHome sites={availableSites} deleted={trash} publications={publicationLinks} activity={cloudRuntime()?.activity||{}} onDuplicate={duplicate} onHistory={store?id=>setHistorySite(availableSites.find(s=>s.id===id)):undefined} actionsDisabled={!saved} onCheckLink={checkLink} onLink={(id,url)=>setSites(all=>all.map(s=>s.id===id?{...s,linkedWebsite:url}:s))} onOpen={changeSite} onPublish={(id,view='publish')=>{setSiteId(id);setPublishing(view)}} onCreate={createSite} onDelete={removeProject} onRestore={restoreDeletedProject} statuses={publicationStatuses}/>:<div className="studio-workspace" data-preview={preview}>
   ...
   <main className="studio-main"><div className="editor-publication-status" role="status"><span className="editor-context"><strong>{siteTitle(site)}</strong><span className="editor-mobile-save">{saveProblem?'저장 확인 필요':!saved?'저장 중…':savedLabel}</span></span><span className="project-status" data-kind={publicationStatuses[site.id]?.kind||'new'} title={publicationStatuses[site.id]?.detail} data-changed={publicationStatuses[site.id]?.needsPublish}>{publicationStatuses[site.id]?.label||'게시 전'}</span></div><PublicationNotice status={publicationStatuses[site.id]} saved={saved} saveProblem={saveProblem} onPublish={()=>{endDrag();setPaletteOpen(false);setPublishing(true)}}/><div className="studio-designbar">{preview&&<PreviewControls width={previewWidth} onWidth={setPreviewWidth} onRestart={()=>setPreviewRestart(n=>n+1)} onShare={()=>setPublishing('review')} shareDisabled={!saved} onReview={()=>setReviewing('preview')}/>}{!preview&&<div className="studio-content-actions"><button className="studio-button prepare-trigger" onClick={openPreparation}><FileText size={14}/>자료 준비</button><button className="studio-button basics-trigger" onClick={openBasics}><ContactRound size={14}/>기본 정보</button><button className="studio-button" onClick={openMedia}><ImagePlus size={14}/>사진·CV</button></div>}<button className="studio-button template-trigger" onClick={openTemplates}><LayoutTemplate size={14}/>템플릿 · {resolveTemplate(site.template).name}</button><label className="font-control"><span>Font</span><select aria-label="Font" value={site.font} onChange={e=>update(s=>({...s,font:e.target.value}))}>{Object.entries(fonts).map(([id,font])=><option key={id} value={id}>{font.name}</option>)}</select></label><div className="theme-control" ref={themePanel}><button className="theme-trigger" aria-expanded={paletteOpen} onClick={()=>setPaletteOpen(v=>!v)}><Palette size={15}/><span>Theme</span><span className="theme-dots"><i style={{background:themes[site.theme].paper}}/><i style={{background:themes[site.theme].accent}}/>{themes[site.theme].detail!==themes[site.theme].accent&&<i style={{background:themes[site.theme].detail}}/>}</span></button>{paletteOpen&&<div className="theme-popover" aria-label="배경색과 포인트색"><div className="theme-heading">배경색 + 포인트색</div>{Object.entries(themes).map(([id,theme])=><button key={id} aria-pressed={site.theme===id} onClick={()=>update(s=>({...s,theme:id}))}><span className="theme-swatch" style={{'--swatch-paper':theme.paper,'--swatch-wash':theme.wash,'--swatch-accent':theme.accent,'--swatch-title':theme.title,'--swatch-detail':theme.detail}}><i/></span><span className="theme-label"><strong>{theme.name}</strong><small>{theme.description}</small></span>{id===site.theme&&<Check size={15}/>}</button>)}</div>}</div></div>
    {!preview&&site.setup&&site.setup.stage!=='edit'&&<div className="setup-resume"><div><strong>{site.setup.stage==='content'?'자료를 채울 차례예요':site.setup.stage==='media'?'사진과 CV를 더할 차례예요':'디자인을 고를 차례예요'}</strong><span>{site.setup.stage==='content'?'AI 답변을 가져오면 페이지를 자동으로 구성해요.':site.setup.stage==='media'?'파일을 넣거나 건너뛰고 디자인을 고르세요.':'같은 내용으로 네 가지 템플릿을 비교해 보세요.'}</span></div><button type="button" className="studio-button" onClick={site.setup.stage==='content'?openPreparation:site.setup.stage==='media'?openMedia:openTemplates}>{site.setup.stage==='content'?'자료 준비 이어가기':site.setup.stage==='media'?'사진·CV 이어가기':'템플릿 고르기'}</button></div>}
    {!preview&&(!site.setup||site.setup.stage==='edit')&&<NextSteps site={site} importResult={importResult} onEdit={editReviewIssue} onPreview={togglePreview} onPreparation={openPreparation} onReview={()=>setReviewing('editor')}/>}
 ...
 </div>{settingMedia&&<SetupMediaDialog key={site.id} site={site} onPhoto={(asset,before)=>saveSetupMedia(s=>setSetupPhoto(s,asset,before))} onCv={(language,asset,before)=>saveSetupMedia(s=>setSetupCv(s,language,asset,before))} onReveal={id=>update(s=>({...s,sections:s.sections.map(section=>section.id===id?{...section,hidden:false}:section)}))} onContinue={continueFromMedia} onClose={()=>setSettingMedia(false)}/>} {creating&&<ProjectSetupDialog onCreate={createPrepared} onClose={()=>setCreating(false)}/>} {templating&&<TemplateDialog key={site.id} site={site} onSave={saveTemplate} onClose={()=>setTemplating(false)}/>} {basicsModal&&<BasicInfoDialog key={basicsModal.site.id} {...basicsModal} onSave={saveBasics} onClose={()=>setBasicsModal(null)}/>} {imageUpload&&<ImageUploadDialog {...imageUpload} onSave={saveImage} onClose={()=>setImageUpload(null)}/>} {importBusy&&<div className="studio-overlay"><div className="publish-dialog" role="status">공용 공간으로 가져오는 중…</div></div>}{publishing&&<PublishDialog key={site.id} site={site} initialView={typeof publishing==='string'?publishing:'publish'} onClose={()=>setPublishing(false)} onCheckpoint={store?checkpointSite:undefined} onReview={()=>{setPublishing(false);setReviewing('publish')}}/>} {reviewing&&<SiteReviewDialog site={site} onClose={()=>{if(reviewing==='publish')setPublishing(true);setReviewing(false)}} onEdit={editReviewIssue} onResolve={issue=>update(s=>resolveReview(s,issue))}/>} {historySite&&<HistoryDialog site={historySite} store={store} onCheckpoint={checkpointSite} onRestore={restoreVersion} onClose={()=>setHistorySite(null)}/>} {preparing&&<PreparationDialog key={site.id} site={site} onApply={applyPrepared} onManual={site.setup?.stage==='content'?manualSetup:undefined} onClose={()=>setPreparing(false)}/>}</div>;
}
