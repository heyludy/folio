...
function App(){
 ...
 return <div className="studio"><div className="studio-background" inert={preparing||publishing||undefined} aria-hidden={preparing||publishing||undefined}>
  ...
  {home?<ProjectHome sites={availableSites} deleted={trash} publications={publicationLinks} onCheckLink={checkLink} onLink={(id,url)=>setSites(all=>all.map(s=>s.id===id?{...s,linkedWebsite:url}:s))} onOpen={changeSite} onPublish={id=>{setSiteId(id);setPublishing(true)}} onCreate={createSite} onDelete={removeProject} onRestore={restoreDeletedProject}/>:<div className="studio-workspace" data-preview={preview}>
