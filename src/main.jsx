...
import {Plus,Eye,ArrowLeft,Download,Check,GripVertical,X,Palette,ContactRound,FileText,Globe,History,LayoutTemplate} from 'lucide-react';
...
import {createPreparedProject,finishContentSetup,startManualSetup,finishTemplateSetup} from './projectSetup';
...
function App({account,onLogout}){
 ...
 const [creating,setCreating]=useState(false),[templating,setTemplating]=useState(false);
 ...
 const loadLatest=async()=>{
  ...
  try{
   ...
   setCreating(false);setTemplating(false);setPreparing(false);setPublishing(false);setImportUndo(null);setSelected(null);setSelectedElement(null);setPendingNavigation(null);setRemovedSection(null);setRemovedLanguage(null);setRemovedProject(null);setBasicsModal(null);setInsertAfter(null);
 ...
 useEffect(()=>{if(ready&&!availableSites.some(s=>s.id===siteId)){setHome(true);setCreating(false);setTemplating(false);setPreparing(false);setPublishing(false);setBasicsModal(null);setPreview(false);setSiteId(availableSites[0]?.id??null)}},[ready,sites,siteId]);
 ...
 const changeSite=id=>{const target=latestSites.current.find(s=>s.id===id);setTemplating(target?.setup?.stage==='template');setPreparing(target?.setup?.stage==='content');setPublishing(false);setPendingNavigation(null);endDrag(false);setSiteId(id);setLang('en');setHome(false);setPreview(false);setSelected(null);setInsertAfter(null);setPaletteOpen(false);canvas.current?.scrollTo({top:0,behavior:'instant'})};
 const goHome=()=>{setTemplating(false);setCreating(false);setPreparing(false);setPublishing(false);setPendingNavigation(null);endDrag(false);setHome(true);setPreview(false);setPaletteOpen(false);setSelected(null);setInsertAfter(null);history.replaceState(null,'',location.pathname+location.search)};
 ...
 const manualSetup=()=>{update(startManualSetup);setPreparing(false);setTemplating(true)};
 ...
 const applyPrepared=async(next,result)=>{
  ...
  setTemplating(merged.setup?.stage==='template');setImportResult({siteId:site.id,...result,at:Date.now()});setRemovedEntry(null);update(()=>merged);setPreparing(false);setPublishing(false);setSelected(null);setSelectedElement(null);setPreview(false);
 ...
 return <div className="studio"><div className="studio-background" inert={!!(creating||templating||basicsModal||preparing||publishing||importBusy||historySite||reviewing||imageUpload)||undefined} aria-hidden={!!(creating||templating||basicsModal||preparing||publishing||importBusy||historySite||reviewing||imageUpload)||undefined}>
