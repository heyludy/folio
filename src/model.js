...
export function siteTitle(site){
 ...
 return (siteLanguages(site).includes('ko')&&profile?.text.ko.title)||profile?.text.en.title||site.name;
}
...
export function section(kind,name,enName,fields={}){
 ...
 return {id:kind,kind,name,enName,fixed:['profile','contact'].includes(kind),nav:true,hidden:false,defaults,
 text:{en:{title:kind==='profile'?'':enName,...fields},ko:{title:kind==='profile'?'':name,...fields}}};
}
export function newSite(){
 return {id:crypto.randomUUID(),name:'새 교수님 사이트',template:DEFAULT_TEMPLATE,languages:['en'],theme:'forest',font:'academic',photo:'',sections:[
 section('profile','소개','About',{college:'',department:'',position:'',body:''}),
 section('research','연구 분야','Research',{topic1:'',text1:'',topic2:'',text2:''}),
 section('publications','주요 논문','Publications',{year1:'',topic1:'',text1:'',year2:'',topic2:'',text2:''}),
 section('cv','학력','Education',{body:''}),section('contact','연락처','Contact',{email:'',organization:'',office:''})]};
}
export function newSection(kind){const item=catalog.find(t=>t[0]===kind);return {...section(kind,item[1],item[2],{body:''}),id:crypto.randomUUID(),nav:false};}
export function reorder(sections,id,beforeId){
 const moving=sections.find(s=>s.id===id),target=sections.find(s=>s.id===beforeId);
 if(!moving||moving.fixed||!target||target.kind==='profile'||id===beforeId)return sections;
 const rest=sections.filter(s=>s.id!==id);rest.splice(rest.findIndex(s=>s.id===beforeId),0,moving);return rest;
}
