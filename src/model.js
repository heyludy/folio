...
export const themes = {
 ...
 teal:{name:'딥틸',description:'아이스블루 · 딥틸 · 라이트골드',paper:'#e7f1f2',wash:'#d7e8eb',line:'#c4d9dd',ink:'#263a3d',muted:'#53666a',accent:'#15616d',title:'#174650',detail:'#45818b',hero:'#15616d',heroTitle:'#f3dda6',heroMuted:'#e7edef'},
...
export function themeStyle(site,lang){
 ...
 return {...Object.fromEntries(['paper','accent','ink','muted','line','wash','title','detail'].map(k=>['--site-'+k,colors[k]])),...(site.template==='color'?{'--site-hero':colors.hero||hero[site.theme]||hero.forest,'--site-hero-title':colors.heroTitle||(site.theme==='forest'?'#ecb499':'#ffffff'),'--site-hero-muted':colors.heroMuted||'#dbe6de'}:{}),'--site-heading':`"${pair[0]}", "Noto Serif KR", serif`,'--site-body':`"${pair[1]}", Pretendard, sans-serif`,'--site-ui':`"${pair[2]||'Pretendard'}", Pretendard, sans-serif`};
}
