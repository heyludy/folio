...
Folio 관리 화면은 GitHub Pages, 교수님 홈페이지는 Cloudflare Pages에 둡니다. 게시 서버는 Cloudflare Worker입니다. 운영 사이트의 편집 내용은 로그인 후 공용 공간에 저장되며 **게시 버튼을 눌렀을 때만** 공개 내용이 바뀝니다.

## 담당자 사용 순서

1. 허용된 Google 계정으로 로그인하고 프로젝트 → **게시**를 엽니다. 공용 공간에 저장된 게시 연결을 자동으로 확인하며 별도 게시 암호는 필요하지 않습니다.
...
Google 로그인 후 같은 공용 프로젝트를 열면 기존 내용·첨부 파일·게시 주소·도메인 연결이 함께 표시됩니다. 이전 브라우저 전용 프로젝트는 원래 브라우저에서 홈의 **이 브라우저 프로젝트 가져오기**로 옮깁니다. 관리자가 가져오면 그 브라우저의 기존 게시 연결도 이전됩니다.

### 클라우드를 사용하지 않는 별도 설치의 게시 연결 복구
...
프로젝트 목록에서 삭제하면 삭제된 프로젝트 목록으로 이동하며 복원할 수 있습니다. 공용 모드에서는 모든 구성원의 목록에 반영됩니다. 공개 사이트는 계속 열립니다. 게시 중단은 별도로 진행하세요.
...
- 운영 사이트는 허용된 Google 계정끼리 공용 프로젝트 권한을 공유합니다. `CLOUD_ENABLED=true`이면 이전 게시 암호는 차단됩니다. 별도 설치에서 클라우드를 끄면 기존 공용 암호 방식을 사용할 수 있습니다. 관리자 키는 운영용 API 호환을 위해 유지합니다.
...
- 공용 초안·사진·PDF는 클라우드에 저장됩니다. 저장 실패 시 해당 브라우저에 복구용 수정본을 유지합니다. 클라우드를 사용하지 않는 별도 설치에서는 브라우저에만 초안을 보관합니다.
