...
4. 홈 카드의 **도메인**, 또는 게시 설정의 **도메인** 탭에서 구매한 도메인을 입력합니다. 연결 후 나타나는 DNS 레코드를 복사해 도메인 관리 화면에 반영하고 **연결 상태 확인**을 누릅니다. DNS/HTTPS 확인이 끝나면 홈 바로가기가 연결된 도메인으로 바뀝니다.
...
- 기존 주소만 바로가기로 연결한 프로젝트에는 게시 관리 권한이 연결되지 않습니다. 도메인은 처음 게시한 브라우저에서 관리하거나 안내된 Cloudflare 설정 화면에서 연결합니다.
